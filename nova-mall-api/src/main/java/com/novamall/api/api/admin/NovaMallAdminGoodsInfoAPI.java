...
import com.novamall.api.api.admin.param.GoodsAddParam;
import com.novamall.api.api.admin.param.GoodsEditParam;
...
public class NovaMallAdminGoodsInfoAPI {

    private static final Logger logger = LoggerFactory.getLogger(NovaMallAdminGoodsInfoAPI.class);
    ...
    public Result list(@RequestParam(required = false) @ApiParam(value = "页码") Integer pageNumber,
        ...
        return ResultGenerator.genSuccessResult(novaMallGoodsService.getNovaMallGoodsPage(pageUtil));
    }
    ...
    public Result save(@RequestBody @Valid GoodsAddParam goodsAddParam, @TokenToAdminUser AdminUserToken adminUser) {
        logger.info("adminUser:{}", adminUser.toString());
        NovaMallGoods novaMallGoods = new NovaMallGoods();
        BeanUtil.copyProperties(goodsAddParam, novaMallGoods);
        String result = novaMallGoodsService.saveNovaMallGoods(novaMallGoods);
    ...
    public Result update(@RequestBody @Valid GoodsEditParam goodsEditParam, @TokenToAdminUser AdminUserToken adminUser) {
        logger.info("adminUser:{}", adminUser.toString());
        NovaMallGoods novaMallGoods = new NovaMallGoods();
        BeanUtil.copyProperties(goodsEditParam, novaMallGoods);
        String result = novaMallGoodsService.updateNovaMallGoods(novaMallGoods);
        if (ServiceResultEnum.SUCCESS.getResult().equals(result)) {
            return ResultGenerator.genSuccessResult();
        } else {
            return ResultGenerator.genFailResult(result);
    ...
    public Result info(@PathVariable("id") Long id, @TokenToAdminUser AdminUserToken adminUser) {
        ...
        NovaMallGoods goods = novaMallGoodsService.getNovaMallGoodsById(id);
        if (goods == null) {
        ...
        thirdCategory = novaMallCategoryService.getGoodsCategoryById(goods.getGoodsCategoryId());
        if (thirdCategory != null) {
            goodsInfo.put("thirdCategory", thirdCategory);
            secondCategory = novaMallCategoryService.getGoodsCategoryById(thirdCategory.getParentId());
            if (secondCategory != null) {
                goodsInfo.put("secondCategory", secondCategory);
                firstCategory = novaMallCategoryService.getGoodsCategoryById(secondCategory.getParentId());
    ...
    public Result delete(@RequestBody BatchIdParam batchIdParam, @PathVariable("sellStatus") int sellStatus, @TokenToAdminUser AdminUserToken adminUser) {
        ...
        if (novaMallGoodsService.batchUpdateSellStatus(batchIdParam.getIds(), sellStatus)) {
            return ResultGenerator.genSuccessResult();
        } else {

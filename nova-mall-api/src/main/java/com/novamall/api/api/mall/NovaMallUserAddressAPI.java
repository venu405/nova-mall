...
import com.novamall.api.api.mall.param.SaveMallUserAddressParam;
import com.novamall.api.api.mall.param.UpdateMallUserAddressParam;
import com.novamall.api.api.mall.vo.NovaMallUserAddressVO;
...
@RestController
@Api(value = "v1", tags = "6.NovaMall商城个人地址相关接口")
@RequestMapping("/api/v1")
public class NovaMallUserAddressAPI {

    @Resource
    private NovaMallUserAddressService mallUserAddressService;

    @GetMapping("/address")
    @ApiOperation(value = "我的收货地址列表", notes = "")
    public Result<List<NovaMallUserAddressVO>> addressList(@TokenToMallUser MallUser loginMallUser) {
    ...
    public Result<NovaMallUserAddressVO> getMallUserAddress(@PathVariable("addressId") Long addressId,
                                                              @TokenToMallUser MallUser loginMallUser) {
        MallUserAddress mallUserAddressById = mallUserAddressService.getMallUserAddressById(addressId);
        NovaMallUserAddressVO novaMallUserAddressVO = new NovaMallUserAddressVO();
        BeanUtil.copyProperties(mallUserAddressById, novaMallUserAddressVO);
        if (!loginMallUser.getUserId().equals(mallUserAddressById.getUserId())) {
            return ResultGenerator.genFailResult(ServiceResultEnum.REQUEST_FORBIDEN_ERROR.getResult());
        }
        return ResultGenerator.genSuccessResult(novaMallUserAddressVO);
    }

...
import com.novamall.api.api.mall.param.SaveOrderParam;
...
import com.novamall.api.service.NovaMallUserAddressService;
...
@RestController
@Api(value = "v1", tags = "7.NovaMall商城订单操作相关接口")
@RequestMapping("/api/v1")
public class NovaMallOrderAPI {
    ...
    @Resource
    private NovaMallUserAddressService novaMallUserAddressService;
    ...
    public Result<String> saveOrder(@ApiParam(value = "订单参数") @RequestBody SaveOrderParam saveOrderParam, @TokenToMallUser MallUser loginMallUser) {
        ...
        List<NovaMallShoppingCartItemVO> itemsForSave = novaMallShoppingCartService.getCartItemsForSettle(Arrays.asList(saveOrderParam.getCartItemIds()), loginMallUser.getUserId());
        if (CollectionUtils.isEmpty(itemsForSave)) {
            //无数据
            NovaMallException.fail("参数异常");
        } else {
            //总价
            for (NovaMallShoppingCartItemVO novaMallShoppingCartItemVO : itemsForSave) {
            ...
            MallUserAddress address = novaMallUserAddressService.getMallUserAddressById(saveOrderParam.getAddressId());
            if (!loginMallUser.getUserId().equals(address.getUserId())) {
                return ResultGenerator.genFailResult(ServiceResultEnum.REQUEST_FORBIDEN_ERROR.getResult());
            }
            //保存订单并返回订单号
            String saveOrderResult = novaMallOrderService.saveOrder(loginMallUser, address, itemsForSave);
    ...
    public Result<NovaMallOrderDetailVO> orderDetailPage(@ApiParam(value = "订单号") @PathVariable("orderNo") String orderNo, @TokenToMallUser MallUser loginMallUser) {
        return ResultGenerator.genSuccessResult(novaMallOrderService.getOrderDetailByOrderNo(orderNo, loginMallUser.getUserId()));
    }

    @GetMapping("/order")
    @ApiOperation(value = "订单列表接口", notes = "传参为页码")
    public Result<PageResult<List<NovaMallOrderListVO>>> orderList(@ApiParam(value = "页码") @RequestParam(required = false) Integer pageNumber,
        ...
        return ResultGenerator.genSuccessResult(novaMallOrderService.getMyOrders(pageUtil));
    }
    ...
    public Result cancelOrder(@ApiParam(value = "订单号") @PathVariable("orderNo") String orderNo, @TokenToMallUser MallUser loginMallUser) {
        String cancelOrderResult = novaMallOrderService.cancelOrder(orderNo, loginMallUser.getUserId());
    ...
    public Result finishOrder(@ApiParam(value = "订单号") @PathVariable("orderNo") String orderNo, @TokenToMallUser MallUser loginMallUser) {
        String finishOrderResult = novaMallOrderService.finishOrder(orderNo, loginMallUser.getUserId());
    ...
    public Result paySuccess(@ApiParam(value = "订单号") @RequestParam("orderNo") String orderNo, @ApiParam(value = "支付方式") @RequestParam("payType") int payType) {
        String payResult = novaMallOrderService.paySuccess(orderNo, payType);
